...
def comp_ecal_hits(ecalhits,ecalhits_ref, evt):
    ...
    for detid in ecalhit_dict:
        if detid not in ecalhit_dict_ref:
            ...
            if detid in ecalhit_dict:
                print "{} detid {} differs in E {} (ref) vs {} (target)".format(evt, detid,ecalhit_dict_ref[detid][0],ecalhit_dict[detid][0])
...
def comp_pfclus(pfclusters,pfclusters_ref):
    ...
    if sum_clus != sum_clus_ref:
        print " PF cluster energy sum diff  {} (target) vs {} (ref) ".format(sum_clus,sum_clus_ref)
...
def comp_ecal_hits(ecalhits,ecalhits_ref, evt):
    ...
    for detid in ecalhit_dict:
        if detid not in ecalhit_dict_ref:
            ...
            if detid in ecalhit_dict:
                print 'detid {}, same recovery E{}'.format(detid,ecalhit_dict_ref[detid][0])
    #for detid in ecalhit_dict_ref:
    #    if detid not in ecalhit_dict:
    #        print "detid {} not in target collection".format(detid)
    #
...
for event_nr,event in enumerate(events):
    ...
    print "checking event ",event_id, str(events_ref.eventAuxiliary().event())
    comp_ecal_hits(ebrechits.product(),ebrechits_ref.product(),event_id)
